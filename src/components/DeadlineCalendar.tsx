    ...
    <div className="bg-white rounded-xl border border-line p-4 space-y-3">
      ...
      <div className="space-y-1.5">
        {deadlines.map((d, i) => {
          ...
          return (
            <div key={i} className="flex items-center justify-between gap-2 border-b border-line last:border-0 pb-1.5 last:pb-0">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-slate-800 truncate">{d.title}</p>
                {d.note && <p className="text-[11px] text-ink-soft">{d.note}</p>}
              </div>
              <div className="text-right shrink-0">
                <p className="text-sm font-bold text-slate-900">{fmt(d.date)}</p>
                <p className="text-[10px] text-ink-soft">{dd >= 0 ? `in ${dd} days` : "passed"}</p>
              </div>
            </div>
          );
        })}
      ...
      {!hasSpecificDates && (
        <p className="text-[10px] text-ink-soft">Detailed dates are shown for the UK today; other regions are coming.</p>
      )}
    </div>
  );
}
