...
import {
  Shield, Download, Upload, Trash2, FileText, Clock, Globe, Key, AlertTriangle, CheckCircle2, LifeBuoy, BarChart2, Briefcase,
} from 'lucide-react';
    ...
    <div className="space-y-4 pb-24">
      ...
      <div className="bg-white rounded-xl border-2 border-slate-200 p-4 space-y-3">
        ...
        <button
          ...
          className="w-full bg-slate-100 border border-slate-200 text-slate-700 py-2.5 rounded-lg text-sm font-semibold hover:bg-slate-200 flex items-center justify-center gap-1.5"
        >
      ...
      <div className="bg-white rounded-xl border-2 border-slate-200 p-4 space-y-3">
        ...
        {!csvRows.length && (
          ...
          >
            ...
            <p className="text-xs text-slate-400 mt-1">Supports .csv exports from any bank</p>
          </div>
        )}
        <input
          ref={csvInputRef}
          ...
          <div className="space-y-1">
            {exportAuditEntries.slice(-5).reverse().map((entry, i) => (
              <div key={i} className="text-xs text-slate-500 flex items-center gap-2 font-medium">
                ...
                <span className="bg-slate-100 border border-slate-200 px-1.5 py-0.5 rounded font-semibold">{entry.type}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border-2 border-slate-200 p-4 space-y-3">
        ...
        <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-[11px] text-slate-500 space-y-1.5 max-h-24 overflow-y-auto leading-relaxed font-medium">
      ...
      <p className="text-center text-[11px] text-slate-400 pt-2 pb-1">{APP_MOTTO}</p>
    </div>
  );
}
